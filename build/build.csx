...
private const string projectName = "LightInject.SignalR";

private const string portableClassLibraryProjectTypeGuid = "{786C830F-07A1-408B-BD7F-6EE04809D6DB}";
private const string csharpProjectTypeGuid = "{FAE04EC0-301F-11D3-BF4B-00C04F79EFBC}";

string pathToSourceFile = @"..\src\LightInject.SignalR\LightInject.SignalR.cs";
...
WriteLine("LightInject.SignalR version {0}" , version);
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package/LightInject.SignalR.Source.nuspec");	
...
{	    
	string pathToMetadataFile = Path.Combine(pathToBuildDirectory, "NugetPackages/Binary/package/LightInject.SignalR.nuspec");
	PatchNugetVersionInfo(pathToMetadataFile, version);
	NuGet.CreatePackage(pathToMetadataFile, pathToBuildDirectory);
}
...
{
	string pathToMetadata = "../src/LightInject.SignalR/NuGet";
	string pathToPackageDirectory = Path.Combine(pathToBuildDirectory, "NugetPackages/Source/package");	
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.SignalR.Source.nuspec");	
	string pathToSourceFile = "tmp/" + frameworkMoniker + "/Source/LightInject.SignalR";
	string pathToDestination = Path.Combine(pathToPackageDirectory, "content/" + packageDirectoryName + "/LightInject.SignalR");
	RoboCopy(pathToSourceFile, pathToDestination, "LightInject.SignalR.cs");
	FileUtils.Rename(Path.Combine(pathToDestination, "LightInject.SignalR.cs"), "LightInject.SignalR.cs.pp");
	ReplaceInFile(@"namespace \S*", "namespace $rootnamespace$.LightInject.SignalR", Path.Combine(pathToDestination, "LightInject.SignalR.cs.pp"));
}
...
private void CopyBinaryFile(string frameworkMoniker, string packageDirectoryName)
{
	...
	RoboCopy(pathToMetadata, pathToPackageDirectory, "LightInject.SignalR.nuspec");
	...
	RoboCopy(pathToBinaryFile, pathToDestination, "LightInject.SignalR.*");
}

private string ResolvePathToBinaryFile(string frameworkMoniker)
{
	var pathToBinaryFile = Directory.GetFiles("tmp/" + frameworkMoniker + "/Binary/LightInject.SignalR/bin/Release","LightInject.SignalR.dll", SearchOption.AllDirectories).First();
...
{
	string pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.SignalR");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.SignalR.Tests");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.SignalR");
	NuGet.Restore(pathToProjectDirectory);
	pathToProjectDirectory = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.SignalR.Tests");
	NuGet.Restore(pathToProjectDirectory);
...
{
	string pathToTestAssembly = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.SignalR.Tests/bin/Release/LightInject.SignalR.Tests.dll");
...
{	
    ...
    MsTest.RunWithCodeCoverage(pathToTestAssembly, pathToPackagesFolder,pathToTestAdapterDirectory, "LightInject.SignalR.dll");
}
...
private void InitializeNugetBuildDirectory(string frameworkMoniker)
{
	...
	{
		var pathToJsonTemplateFile = Path.Combine(pathToBinary, "LightInject.SignalR/project.json_");
		var pathToJsonFile = Path.Combine(pathToBinary, "LightInject.SignalR/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
		pathToJsonTemplateFile = Path.Combine(pathToSource, "LightInject.SignalR/project.json_");
		pathToJsonFile = Path.Combine(pathToSource, "LightInject.SignalR/project.json");
		File.Move(pathToJsonTemplateFile, pathToJsonFile);
	}				  
}
...
private void Internalize(string frameworkMoniker)
{
	...
	string pathToSourceFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + "/Source/LightInject.SignalR/LightInject.SignalR.cs");
...
{
	string pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.SignalR/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
	
	pathToPackagesConfig = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.SignalR/packages.config");
	ReplaceInFile(@"(targetFramework=\"").*(\"".*)", "$1"+ frameworkMoniker + "$2", pathToPackagesConfig);
}
...
{	
	var pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Binary\LightInject.SignalR\Properties\AssemblyInfo.cs");	
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);
	pathToAssemblyInfo = Path.Combine(pathToBuildDirectory, framework + @"\Source\LightInject.SignalR\Properties\AssemblyInfo.cs");
	PatchAssemblyVersionInfo(version, fileVersion, framework, pathToAssemblyInfo);	
...
private void PatchInternalsVisibleToAttribute(string pathToAssemblyInfo)
{
	...
	sb.AppendLine(@"[assembly: InternalsVisibleTo(""LightInject.SignalR.Tests"")]");
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Binary/LightInject.SignalR/LightInject.SignalR.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"/Source/LightInject.SignalR/LightInject.SignalR.csproj");
	PatchProjectFile(frameworkMoniker, frameworkVersion, pathToProjectFile);
...
private void SetTargetFrameworkProfile()
{
	...
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, @"PCL_111/Binary/LightInject.SignalR/LightInject.SignalR.csproj");
	XDocument xmlDocument = XDocument.Load(pathToProjectFile);	
...
{
	var pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Binary\LightInject.SignalR.Tests\LightInject.SignalR.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);	
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
	pathToProjectFile = Path.Combine(pathToBuildDirectory, frameworkMoniker + @"\Source\LightInject.SignalR.Tests\LightInject.SignalR.Tests.csproj");
	WriteLine("Patching {0} ", pathToProjectFile);
	SetProjectFrameworkMoniker(frameworkMoniker, pathToProjectFile);
}